...
| `crates/xenon_terminal/assets/one.json` | zed-industries/zed | ced90fc (v1.9.0) | GPL-3.0-or-later | One theme family (`assets/themes/one/one.json`); One Dark surfaces set to pure black; One Light canvas/window retuned off the gray slab. |
...
| `crates/xenon_settings/fonts/lilex/*` | mishamyrt/Lilex (via zed `assets/fonts/lilex` at ced90fc) | OFL-1.1 | Bundled editor/terminal face. GPUI maps `.ZedMono` → Lilex; we load the TTFs so the picker resolves. |
