...
| `crates/xero_terminal/src/view/mod.rs` | zed-industries/zed | ced90fc (v1.9.0) | GPL-3.0-or-later | PTY spawn + subscribe patterns adapted from `terminal_view.rs` and `project/src/terminals.rs`; Cmd-hover link detection drives zed's `Terminal::mouse_move`, and the grid position-to-cell mapping mirrors zed's `grid_point`. Xenon-only helpers live in `view/attention.rs` and `view/paths.rs`. |
